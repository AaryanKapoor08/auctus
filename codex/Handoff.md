...
**Status:** G2 complete; G3 ready to start after G2 commit/PR proof is recorded
...
1. Commit G2 with format `refactor(restructure): isolate legacy demo surface`.
2. Update the G2 Proof Log ref from `pending G2 commit` to the actual commit hash, then commit that docs update if needed.
3. Open/push the G2 branch through the protected workflow as required.
4. Start G3 only after G2 proof is recorded. G3 next code tasks are `lib/env.ts`, Vitest + `npm test` / `npm run test:watch`, and a sanity contract import test.
